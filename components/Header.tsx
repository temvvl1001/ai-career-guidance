...
import { Menu, User, LogOut, Languages, Sun, Moon } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              {user ? (
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 p-2 rounded-lg bg-dark-800 hover:bg-dark-700"
                  >
                    <User className="w-5 h-5 text-accent-purple" />
                    <span className="hidden sm:inline text-sm">
                      {user?.name || user?.email?.split("@")[0]}
              ...
              <button
                onClick={toggleLanguage}
                className="hidden sm:inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-dark-800 text-xs"
              >
                <Languages className="w-3 h-3" />
                <span>{isMn ? "MN" : "EN"}</span>
              </button>
              ...
              <nav className="flex flex-col gap-2">
                {navLinks.map((link) => (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setMenuOpen(false)}
                    className="px-4 py-2 rounded-lg hover:bg-dark-700"
                  >
                    {link.label}
                  </Link>
                ))}
